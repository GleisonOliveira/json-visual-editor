...
- **THEN** a "Pular para o conteúdo" link becomes visible and, when activated, moves focus to the main content area

#### Scenario: CodeMirror aria-label changes on mode switch
- **WHEN** the user enters edit mode via "Editar JSON"
- **THEN** the CodeMirror editor's aria-label changes from "Visualização de JSON" to "Editor de JSON"
